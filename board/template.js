...
function renderAvatarBigCard(taskAvatars) {
    ...
    for (let i = 0; i < taskAvatars.length; i++) {
        ...
        avatar += `<div class="d-flex-start-c gap-5 mh-5">
                    <span class="avatar-card" style="background-color: ${taskAvatars[i].bgcolor};">${initials}</span>
                    <p>${capitalizedName}</p>
                </div>`
    }
    return avatar;
...
function renderSubtaskBigCard(task, subtasks) {
    ...
    subtask += `<p>Subtasks:</p><div class="mh-2">`;
    for (let i = 0; i < subtasks.length; i++) {
        if (subtasks[i].completed == true) {
            subtask += `<div class="d-flex-sb-c gap-15">
